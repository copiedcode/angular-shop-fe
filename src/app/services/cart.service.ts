...
export class CartService {
  ...
  // Data variable to store cart information on the server (frontend)
  ...
  constructor(private http: HttpClient,
    ...
    if(info != null && info != undefined && info.prodData[0].incart != 0){
      ...
      // Loop though each entry and put it in the cartDataServer object
  ...
  AddProductToCart(id: number, quantity?: number){
    this.productService.getSingleProduct(id).subscribe(prod =>{
      ...
      // 2. Cart has some items already
        ...
        let index = this.cartDataServer.data.findIndex(p => p.product.id == prod.id); // -1 or a positiv value
        // 2.a. item is already in the cart (index is a positive value)
        if(index != -1){
